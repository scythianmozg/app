...
for row in movies_table: # перебирается каждый внутренний список
    for elem in row: # в очередном внутреннем списке перебирается каждый элемент
        print(elem, end='   ') # печатается очередной элемент, в конец добавляются три пробела, без перевода на новую строку
    print() # после того, как все элементы очередной строки напечатаны, вывод переводится на новую строку